...
export default function TopNav() {
  ...
  const fetchPending = async () => {
    try {
      const [deRes, bookRes] = await Promise.all([
        fetch('/api/de-requests?status=pending'),
        fetch('/api/book-requests?status=pending'),
      ]);
      const [deData, bookData] = await Promise.all([deRes.json(), bookRes.json()]);
      const de: DEUser[] = (deData.data || []).map((u: DEUser) => ({ ...u, portal: 'data_entry' as const }));
      const books: DEUser[] = (bookData.data || []).map((u: DEUser) => ({ ...u, portal: 'books' as const }));
      setPendingUsers([...de, ...books]);
    } catch {
      // ignore
    }
  };

  const handleAction = async (id: string, action: 'approve' | 'reject', portal: 'data_entry' | 'books') => {
    setActionLoading(id);
    try {
      await fetch(portal === 'books' ? '/api/book-requests' : '/api/de-requests', {
        method: 'PATCH',
            ...
            <div className="absolute right-0 top-12 bg-white border border-slate-200 rounded-xl shadow-xl w-80 z-50 overflow-hidden">
              ...
              {pendingUsers.length === 0 ? (
                ...
                <div className="max-h-72 overflow-y-auto divide-y divide-slate-100">
                  {pendingUsers.map((u) => (
                    <div key={`${u.portal}-${u.id}`} className="px-4 py-3">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border ${
                          ...
                          {u.portal === 'books' ? 'BOOKS' : 'DATA ENTRY'}
                        </span>
                      </div>
                      <p className="text-[11px] text-slate-500 mb-2">{u.email}</p>
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleAction(u.id, 'approve', u.portal)}
                        ...
                        <button
                          onClick={() => handleAction(u.id, 'reject', u.portal)}
                          disabled={actionLoading === u.id}
